...
object KMS {
  ...
  def listAliases(): IO[fs2.Stream[IO, AliasListEntry]] =
    for {
  ...
  def listKeys(): IO[fs2.Stream[IO, KeyListEntry]] =
    for {
  ...
  def createAlias(alias: String, keyId: String): IO[CreateAliasResponse] =
  ...
  /** Note this  uses default Symmetric encrypr/descrypt WITHOUT defining policies
    ...
    */
  def createSymmetricKey(desc: String, tags: OTags): IO[CreateKeyResponse] =
    IOU.toIO(
      client.createKey(
  ...
  /** API Based Encryption --- not sure how much is remote. Client side we can use the AWS Crypto lib.
    ...
    */
  def encrypt(msg: String, keyId: String, context: Map[String, String]): IO[EncryptResponse] =
    IOU.toIO(
      client.encrypt(
  ...
  def decrypt(bytes: Array[Byte], keyId: String, context: Map[String, String]): IO[DecryptResponse] =
    IOU.toIO(
      client.decrypt(
