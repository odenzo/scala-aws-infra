...
object EKS {
  ...
  def listClusters(): IO[Stream[IO, String]] = {
    for {
  ...
  def deleteNodegroup(clusterName: String, ngName: String): IO[Nodegroup] = {
    IOU
  ...
  def listNodegroups(clusterName: String): IO[List[String]] = {
    for {
  ...
  def describeNodegroup(clusterName: String, nodegroupName: String): IO[Nodegroup] = {
    IOU
  ...
  def isNodegroupReady(ng: Nodegroup): IO[Nodegroup] = {
  ...
  def executeCreateCluster(rq: CreateClusterRequest): IO[Cluster] = {
    IOU.toIO(client.createCluster(rq)).map(_.cluster)
  }

  def waitForClusterAvailable(clusterName: String): IO[Cluster] = {
  ...
  def attachStandardEksServiceRolePolicies(roleName: String): IO[Unit] = {
    for {
  ...
  /** Attaches EKSClusterPolicy and some inline policies */
  def attachStandardEksWorkerNodeRolePolicies(roleName: String): IO[Unit] = {
    for {
