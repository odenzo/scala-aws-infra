...
object FS2Utils {
  ...
  def toList[T, U](publisher: => Publisher[T]): IO[List[T]] = publisher.toStream[IO].compile.toList

  /** Takes a tunk and suspends running it. Usually we are already in an IO but.... */
  def toStream[T](publisher: => Publisher[T]): IO[Stream[IO, T]] = {
  ...
  def toBurstStream[T, U](publisher: => Publisher[T])(fn: T => IterableOnce[U]): IO[Stream[IO, U]] = {
    toStream(publisher).map { s => s.map(t => fn(t)).flatMap(u => fs2.Stream.emits(u.iterator.toSeq)) }
  }

  def toBurstList[T, U](publisher: => Publisher[T])(fn: T => IterableOnce[U]): IO[List[U]] = {
  ...
  /** This will retry, so take care to align your indempotency tokens as needed.
    ...
    */
  def uniformRetry[A](interval: FiniteDuration, maxAttempts: Int)(fn: => IO[A]): IO[A] = {
  ...
  def backoffRetry[A](delay: FiniteDuration, maxAttempts: Int)(fn: => IO[A]): IO[A] = {
