...
object SOPS {
  ...
  def decryptJson(fromFile: File): IO[Json] = {
    IO.delay {
      os
        .proc(Seq("/usr/local/bin/sops", "--input-type", "json", "--output-type", "json", "-d", fromFile.getCanonicalPath))
        .call(check = true)
        .out
        .text(Codec.UTF8)
  ...
  def decryptYaml(fromFile: File): IO[Json] = {
    IO.delay {
      os
        .proc(Seq("/usr/local/bin/sops", "--input-type", "yaml", "--output-type", "json", "-d", fromFile.getCanonicalPath))
        .call(check = true)
        .out
        .text(Codec.UTF8)
    }.flatMap(txt => IO.fromEither(io.circe.parser.parse(txt)))
